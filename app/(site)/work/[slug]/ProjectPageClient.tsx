...
export function ProjectPageClient({ project, allMedia }: ProjectPageClientProps) {
  // Simple scroll handlers without circular dependency
  const handleRailScroll = useCallback((progress: number) => {
    // For now, we'll just log the progress
    // In a real implementation, you might want to sync this with the gallery
    console.log('Rail scroll progress:', progress)
  }, [])

  const handleGalleryScroll = useCallback((progress: number) => {
    // For now, we'll just log the progress
    // In a real implementation, you might want to sync this with the rail
    console.log('Gallery scroll progress:', progress)
  }, [])
            ...
            <div className="md:hidden space-y-8">
              {allMedia.map((item, index) => (
                <div key={index} className="relative">
                  ...
                  />
                  {item.caption && (
                    <div className="mt-2 text-sm text-gray-600 text-center font-body">
                      {item.caption}
                    </div>
                  )}
                </div>
                ...
                {project.description && (
                  <div className="prose prose-lg">
                    {project.description.split('\n').map((paragraph, index) => (
                      <p key={index} className="mb-4 text-gray-700 font-body">
                        {paragraph}
                      </p>
                    ))}
                  </div>
